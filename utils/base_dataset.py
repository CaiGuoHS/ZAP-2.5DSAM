...
class BaseVolumeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return img_aug, seg_aug, np.array(img_vol.header.get_zooms())[self.spatial_index], img_path, affine
        ...
        if self.split == "train":
            ...
            transforms.extend(
                [
                    ...
                    SpatialPadd(
                        keys=["image", "label"],
                        spatial_size=[round(i * 1.5) for i in self.rand_crop_spatial_size],
                    ),
                    RandCropByPosNegLabeld(
                        keys=["image", "label"],
                        spatial_size=[round(i * 1.5) for i in self.rand_crop_spatial_size],
